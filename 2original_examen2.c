...
char alumno[100] = "Arantxa Angulo Flores";
...
/* ----------  INICIO RESPUESTA:  --------------- */
node * cambiarAPrimero(node* primero){
  node * cubeta = primero; // Placeholder/copia del primer nodo

  while(cubeta->next->next){
    cubeta = cubeta->next;
  }

  node * ultimo = cubeta->next;
  cubeta->next = NULL;
  ultimo->next = primero;
  return ultimo;
}
